...
// 98-byte payload requires at least DR3 (SF9/125 kHz) in EU868.
// DR0–DR2 max payloads are 51 / 51 / 51 bytes — below our frame size.
#define THREE_PHASE_MIN_DR DR_3
...
void prepareLoraFrame(bool user_btn_pressed) {
  ...
  index = add_stats_values_to_app_data(INST_L2_DECI_V_STATS_INDEX, index);
  index = add_stats_values_to_app_data(INST_L3_DECI_V_STATS_INDEX, index);
  index = add_stats_values_to_app_data(INST_L2_CENTI_A_STATS_INDEX, index);
  index = add_stats_values_to_app_data(INST_L3_CENTI_A_STATS_INDEX, index);
...
void enforce_min_dr() {
  // Ensure the data rate is at least DR3 before transmitting the 98-byte three-phase frame.
  // ADR may have stepped the DR down to DR0/DR1/DR2, which cannot carry this payload.
  MibRequestConfirm_t mibReq;
  mibReq.Type = MIB_CHANNELS_DATARATE;
  LoRaMacMibGetRequestConfirm(&mibReq);
  if (mibReq.Param.ChannelsDatarate < THREE_PHASE_MIN_DR) {
    Serial.printf("DR%d too low for 98-byte payload, forcing DR%d\n",
                  mibReq.Param.ChannelsDatarate, THREE_PHASE_MIN_DR);
    mibReq.Param.ChannelsDatarate = THREE_PHASE_MIN_DR;

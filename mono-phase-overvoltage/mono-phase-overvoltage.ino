...
/*This is the frequency of the pulse status packets
trasmission append every PULSE_HISTORY_INTERVAL * txInterval */
...
void prepareLoraFrame(bool user_btn_pressed) {
  /*
     ...
     * Frame format (v2):
      ...
      | Humidity          | 8           | 61    | 0-100%                    |
      | L1-over-253V      | 16          | 62    | Stage1 count (optional*)  |
      | L1-over-264.5V    | 16          | 64    | Stage2 count (optional*)  |
      | L1-max-consec-253 | 16          | 66    | Max streak (optional*)    |
      +-------------------+-------------+-------+---------------------------+
      * Overvoltage fields only included if L1-over-253V > 0
      * Thresholds per Synergrid C10/11 / EN 50549-1:
        - Stage1: 253V (1.10×Un), 1.5s trip
        - Stage2: 264.5V (1.15×Un), instant trip
      Frame size: 62 bytes (normal) or 68 bytes (with overvoltage data)
     */
  uint8_t index = 0;
  ...
  // Overvoltage data (Synergrid C10/11 thresholds: 253V stage1, 264.5V stage2)
  // Only include if there were any overvoltage events
  if (get_nb_samples_over_stage1(OVER_VOLTAGE_PHASE_1_INDEX) > 0){
    index = add_uint_16_to_app_data(get_nb_samples_over_stage1(OVER_VOLTAGE_PHASE_1_INDEX), index);
    index = add_uint_16_to_app_data(get_nb_samples_over_stage2(OVER_VOLTAGE_PHASE_1_INDEX), index);
    index = add_uint_16_to_app_data(get_max_consecutive_over_stage1(OVER_VOLTAGE_PHASE_1_INDEX), index);
  }
...
void process_elec_message() {
  ...
  for (int i = 0; i < TELE_NUMFIELDS; i++) {
    ...
    if (i == INST_L1_DECI_V_INDEX){
      // voltage field, check for overvoltage (Synergrid C10/11 compliance)
      update_over_voltage_counter(OVER_VOLTAGE_PHASE_1_INDEX, elec_field_int_value(i));
    }
  }
